...
## Modelando resultados de partidos de fútbol: Iteración 1
¿Cómo se distribuye el número de goles de cada equipo en un partido de fútbol? 
...
$$log(\lambda) = baseline + skill_i − skill_j$$
donde la línea de base (baseline) es el promedio logarítmico del número de goles cuando ambos equipos son igual de buenos. El resultado de goles de un partido entre el equipo local $i$ y el equipo visitante $j$ se modela como:
$$HomeGoals_{i,j} \sim Poison(\lambda_{home,i,j})$$
$$AwayGoals_{i,j} \sim Poison(\lambda_{away,i,j})$$
$$log(\lambda_{home,i,j}) = baseline + skill_i − skill_j$$
$$log(\lambda_{away,i,j}) = baseline + skill_j − skill_i$$
Para contar con un modelo bayesiano agregaremos algunas distribuciones a priori sobre la línea de base (baseline) y la habilidad (skill) de todos los $n$ equipos:
$$baseline \sim Normal(0, 4^2)$$
$$skill_{1...n} \sim Normal(\mu_{teams}, \sigma^2_{teams})$$
...
plot_pred_comp1 <- function(home_team, away_team, ms) {
  # Simulates and plots game goals scores using the MCMC samples from the model1
  par(mfrow = c(2, 4))

...
Aquí descubrimos que sin importar que cambiemos de lugar a los equipos, los datos simulados muestran que el `Chelsea` gana por mímino, salvo que ahora es equipo visitante, mientras que los datos históricos muestran lo mismo que el `Chelsea` gana aún cuando es equipo visitante. Derivado de la diferencia mínima que observamos en nuestro modelo, consideramos que predice con la precisión requerida, ya que no considera la ventaja de ser el equipo local. Lo descrito anteriormente, lo analizaremos a partir del modelo descrito a continuación:

## Iteración 2

Para tener en cuenta la ventaja de jugar en casa, cambiaremos nuestro modelo inicial donde dividiremos la línea base (baseline) en dos componentes:
1\. Una línea base (home_baseline) local
2\. Una línea base (away_baseline) visitante.
El siguiente modelo `JAGS` aplica el cambio mencionado anteriormente:
...
```{r fit_model2, warning=FALSE, message=FALSE, echo=FALSE, results='hide'}
...
summary_combined_df2 <- left_join(summary_df2, quantiles_df2, by = "parameter")
...
plot_pred_comp2 <- function(home_team, away_team, ms) {
  ...
  home_goals_sim <- rpois(nrow(ms), exp(home_baseline + home_skill - away_skill))
  away_goals_sim <- rpois(nrow(ms), exp(away_baseline + away_skill - home_skill))
...
Ahora los resultados se acercan más a los datos históricos, ya que tanto el `Tottenham` como el `Chelsea` tienen más probabilidades de ganar cuando juegan como locales. En este punto del proceso de modelización, decidimos ajustar la habilidad de los equipos en función de la temporada, ya que los equipos pueden cambiar su habilidad de un año a otro. A continuación, se muestra el modelo de la tercera iteración.
...
```{r fit_model3, warning=FALSE, message=FALSE, echo=FALSE, results='hide'}
...
team_skill <- (team_skill - rowMeans(team_skill)) + matrix_samples3[, "home_baseline[5]"]

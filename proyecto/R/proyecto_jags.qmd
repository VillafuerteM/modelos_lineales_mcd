...
```{r base_to_predict, echo=FALSE, warning=FALSE, message=FALSE}
games_pred <- read_csv("data/Games_Pred.csv")
...
games_pred <- games_pred |>
  mutate(
    ...
    YearMatch = as.character(YearMatch),
    MonthMatch = as.character(MonthMatch),
    DayMatch = as.character(DayMatch),
    MatchResult = as.numeric(MatchResult)
  )

d2 <- bind_rows(d, games_pred)
```
...
data_list3 <- list(HomeGoals = d2$HomeGoals, AwayGoals = d2$AwayGoals, 
                  HomeTeam = as.numeric(factor(d2$HomeTeam, levels=teams)),
                  AwayTeam = as.numeric(factor(d2$AwayTeam, levels=teams)),
                  Season = as.numeric(factor(d2$Season, levels=seasons)),
                  n_teams = length(teams), n_games = nrow(d2), 
                  n_seasons = length(seasons))

model3_fit <- jags.model(textConnection(model3), data=data_list3, 
...
```{r results_forecast, results='asis', echo=FALSE}
pl_forecast <- d2[is.na(d2$HomeGoals), c("Season", "HomeTeam", "AwayTeam")]
m3_forecast <- m3_pred[is.na(d2$HomeGoals),] 
pl_forecast$mean_home_goals <- round(m3_forecast[,"mean_home_goal"], 1) 
pl_forecast$mean_away_goals <- round(m3_forecast[,"mean_away_goal"], 1)
pl_forecast$mode_home_goals <- m3_forecast[,"mode_home_goal"] 
pl_forecast$mode_away_goals <- m3_forecast[,"mode_away_goal"]
pl_forecast$predicted_winner <- ifelse(m3_forecast[ , "match_result"] == 1, pl_forecast$HomeTeam, 
                                           ifelse(m3_forecast[ , "match_result"] == -1, pl_forecast$AwayTeam, "Draw"))

rownames(pl_forecast) <- NULL
kable(pl_forecast, format = "markdown", align = "c")
```
...
En el transcurso de este análisis, se ha trabajado con el objetivo de modelar los resultados de los partidos en el conjunto de datos, y se han logrado alcanzar importantes hitos:

-Modelado de resultados y probabilidades: Se ha desarrollado un modelo capaz de calcular las probabilidades de posibles resultados de goles en partidos futuros de la Premier League. Esta capacidad es fundamental para la toma de decisiones informadas en el ámbito deportivo y puede ser de gran utilidad para diversos fines, ya sea en el rendimiento de los equipos, o simplemente para la diversión de los aficionados. -Producción de clasificaciones confiables: Además de la predicción de resultados de partidos, se ha obtenido una clasificación confiable de los equipos. Esta información es valiosa para evaluar el desempeño de los equipos a lo largo del tiempo y puede servir como base para análisis más profundos y decisiones estratégicas. -Predicción de resultados recientes: Se logró predecir con éxito los resultados de los últimos 20 partidos de la Premier League, lo que demuestra la capacidad predictiva y la validez del modelo desarrollado. Estos resultados son prometedores y sugieren que el modelo podría ser útil para predecir resultados con un grado de precisión relevante.

Sin embargo, es importante mencionar algunas limitaciones y áreas de mejora para futuros estudios:

-Limitaciones encontradas: Se encontraron ciertas limitaciones en el alcance del proyecto, como la imposibilidad de elevar la complejidad del modelo debido a restricciones computacionales o la necesidad de correr el modelo particionad, lo cual debe tenerse en cuenta al interpretar los resultados y al considerar futuras investigaciones. -Futuras vías de ampliación del estudio: Para ampliar este estudio, se podría considerar la inclusión de más variables relevantes, como lesiones de jugadores clave, condiciones climáticas, estadísticas históricas adicionales, o inclusive el criterio experto de algunos comentaristas o críticos del deporte. Asimismo, explorar diferentes técnicas de modelado o validar el modelo con datos de temporadas futuras puede mejorar aún más la precisión y la robustez del análisis.

En resumen, este proyecto ha sentado las bases para un análisis profundo y continuo del rendimiento y los resultados en este deporte, proporcionando tanto herramientas predictivas como insights útiles para la toma de decisiones estratégicas, al menos de la Premier League.

\newpage

# Fuentes

•⁠ ⁠Bååth, R. (2015). Modeling Match Results in Soccer using a Hierarchical Bayesian Poisson Model. LUCS Minor, (18), Lund University Cognitive Science. ISSN 1104-1609. •⁠ ⁠Football-Data.co.uk. (n.d.). England Historical Football Results. Recuperado de https://www.football-data.co.uk/englandm.php •⁠ ⁠Kruschke, J. K. (2014). Doing Bayesian data analysis: A tutorial with R, JAGS, and Stan (2nd ed.). Academic Press.

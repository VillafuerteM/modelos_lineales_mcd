...
a.  Mostrar que el número de sucesiones binarias de longitud m sin 1’s
    ...
    $\\f_1 = c_1 r_1^1 + c_2 r_2^1 = 1\\$ resolving this linear problema
    leads to: $\\ c_1 = \frac{1}{\sqrt{5}}$
    $\\ c_2 =-\\frac{1}{\\sqrt{5}}$

number of sequences:
$$a(n)=f(n+2)= \frac{1}{\sqrt{5}}(\frac{1+\sqrt5}{2})^2-\frac{1}{\sqrt{5}}(\frac{1-\sqrt5}{2})^2$$

b.  Sea pk,m el número de buenas sucesiones de longitud m con
    exactamente k 1’s. Mostrar que $$\\\begin{pmatrix} m-k+1 \\ k \end{pmatrix}$$ , k = 0, 1,..., ceiling(m/2)

```{r}
N <- 3
m <- seq(1, N)
k <- seq(0, ceiling(N/2))
sol <- matrix(0, nrow = N, ncol=3)
for (midx in m){
  for (kidx in seq(1:length(k))){
    num <- factorial(m[midx]-k[kidx]+1)
    den <- factorial(k[kidx])*factorial(m[midx]-k[kidx]+1-k[kidx])
    sol[midx, 1] <- num/den
    sol[midx, 2] <- m[midx]
    sol[midx, 3] <- k[kidx]
  }
...
c.  Sea $µ_m$ el número esperados de 1’s en una buena sucesión de longitud
    m bajo la distribución uniforme. Encontrar µm para m = 10, 100,
    1000. 
